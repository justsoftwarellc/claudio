...
It's stored in [`~/.claudio/config.yml`](#claudioyml-and-configyml) as `editor:`, machine-level rather than per-repo, since your editor shouldn't arrive with someone else's clone.
...
editor: code                    # what `claudio open` launches; bare binary or absolute path
                                #   set it with: claudio config set editor <binary>
resources:
  memory: 6g
  cpus: 4
  pids: 512
...
`editor` has no default; every other command works without it. Prefer `claudio config set editor` over hand-editing — it checks the binary resolves on `PATH` first.
