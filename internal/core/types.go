...
type InstanceView struct {
	...
	ContainerStatus  string // Docker's own status string: "running", "exited", ...
	OOMKilled        bool   // surfaced distinctly — see ROD-112: a bare
	// "stopped" is a worse failure mode than an explained one.
...
type UntrackedContainer struct {
	...
	RepoURL       string // from the claudio.repo label, if present
	CreatedAt     int64  // from the claudio.created_at label, if present
	Running       bool
}
